...
sourcepage = 'Gebruiker:Vinkje83/WTA150-d'
destination= 'User:Vinkje83/WTA150-d/results'
...
class Dame:
        def __init__(self, rij):
                ...
                self.Qid = ''
        def load_dubbelhoogstepositie(self, site):
                p = pywikibot.Page(site, self.naam)
                wikicode = mwparserfromhell.parse(p.text)
                "Dubbelhoogstepositie  Infobox tennisspeler"
                ...
                if len(infoboxes) == 0:
                   ...
                   return
                if ('Dubbelhoogstepositie' in infoboxes[0]):
                  positie_text = infoboxes[0].get('Dubbelhoogstepositie').value.strip_code() # "17. (18 april 2018)"
                  if (positie_text != ''):
...
def laadtennistabel(site):
        ...
        for dame in dames:
          try:
            print(dame.naam)
            dame.load_dubbelhoogstepositie(site)
          except:
            pass  
        return dames
